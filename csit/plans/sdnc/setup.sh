...
do
    ...
    echo ${mname} ${bname} aai-disabled sync >> ${WORKSPACE}/archives/gra.patch.graph.versions
done
docker cp ${WORKSPACE}/archives/gra.patch.graph.versions ${SDNC_CONTAINER_NAME}:/tmp/gra.patch/graph.versions
docker exec -ti ${SDNC_CONTAINER_NAME} /opt/onap/sdnc/svclogic/bin/svclogic.sh install /tmp/gra.patch /opt/onap/sdnc/svclogic/config/svclogic.properties
