...
def extract_content(file, password, count):
    ...
    if (file.endswith('.jks')):
        ...
        jks_cmd = 'keytool -importkeystore -srckeystore {} -destkeystore {} -srcstoretype JKS -srcstorepass {} -deststoretype PKCS12 -deststorepass {}'.format(file, p12_file, password, password)
        logging.debug("Converting %s into p12 format", file)
        os.system(jks_cmd)
        file = p12_file

    clcrt_cmd = 'openssl pkcs12 -in {} -clcerts -nokeys  -passin pass:{}'.format(file, password)
    clkey_cmd = 'openssl pkcs12 -in {}  -nocerts -nodes -passin pass:{}'.format(file, password)
    trust_file = file.split('/')[2] + '.trust'
    trustCerts_cmd = 'openssl pkcs12 -in {} -out {} -cacerts -nokeys -passin pass:{} '.format(file, Path + '/' + trust_file, password)
...
def lookforfiles():
    ...
    if count > 0:
        ...
        logging.debug("No valid files found under cert directory %s", Path)
...
def readCertProperties():
    ...
    if connected:
        ...
        if os.path.isfile(Path + "/certs.properties"):
            ...
            logging.info("Looking for cert files under cert directory")
            lookforfiles()
