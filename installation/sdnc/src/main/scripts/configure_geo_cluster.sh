...
function create_strings
{
    ...
    # new file
        ds[$count]=\\\"akka.tcp:\\/\\/opendaylight-cluster-data@${ip}:3026$(((${count} - 1) % 3 + 4))\\\"
...
function get_cli_params
{
    ...
    CONTROLLER_PORT="3026$(((${INDEX} - 1) % 3 + 4))"
    MY_IP=$(hostname -i)
}
...
function modify_conf_files
{
    ...
    sed -i -e "s/port[ ]*=.*/port = ${CONTROLLER_PORT}/"  ${AKKACONF}
    sed -i -e "s/netty.tcp[ ]*{.*/netty.tcp { \n\tbind-hostname = \"${MY_IP}\"\n\tbind-port = 2550\n/" ${AKKACONF}
